...
// ----------------------------------------------------------------------------
function isMysqlInitialized(dataDir) {
...
// ----------------------------------------------------------------------------
function spawnMysqlServer(mysqldExe, baseDir, dataDir) {
    console.log(`[Electron Main] Menjalankan mysqld.exe pada port ${MYSQL_PORT}...`);
    mysqlProcess = spawn(mysqldExe, [
        `--datadir=${dataDir}`,
        `--basedir=${baseDir}`,
        `--port=${MYSQL_PORT}`,
    ...
    mysqlProcess.on("exit", (code, signal) => {
        ...
        mysqlProcess = null;
        if (!isShuttingDown) {
            dialog.showErrorBox("Koneksi Database Terputus", "Server database MySQL portable tiba-tiba berhenti berjalan.");
            app.quit();
        }
    });
}
...
// ----------------------------------------------------------------------------
async function waitForMySQLReady(timeoutMs = 15000) {
...
// ----------------------------------------------------------------------------
async function waitForHttpServer(initialUrl, timeoutMs = 25000) {
...
function startProductionNodeServer() {
...
app.whenReady().then(async () => {
    try {
        ...
        if (!isMysqlReady) {
            dialog.showErrorBox("Koneksi MySQL Gagal", `Gagal menghubungkan ke MySQL portable pada port ${MYSQL_PORT}.\nPastikan port ${MYSQL_PORT} tidak sedang diblokir atau digunakan oleh aplikasi lain.`);
            await cleanShutdown();
            app.quit();
            return;
        }
        ...
        if (isDev) {
            ...
            if (!devReady) {
                dialog.showErrorBox("Dev Server Tidak Merespons", `Tidak dapat menghubungi Vite dev server pada ${targetUrl}.\nPastikan perintah 'npm run dev' sedang berjalan.`);
                await cleanShutdown();
                app.quit();
                return;
            }
            ...
            if (!serverReady) {
                dialog.showErrorBox("Server Aplikasi Gagal Memulai", `Server internal Node.js tidak merespons pada ${targetUrl}.`);
                await cleanShutdown();
                app.quit();
                return;
            }
